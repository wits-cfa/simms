...
Add thermal noise
~~~~~~~~~~~~~~~~~~~
There are to ways to add thermal noise to the generated visibilities. You can either provide a System Equivalent Flux Density (SEFD; in Jy) via the ``--sefd`` option, or via the ``--tsys-over-eta`` option which specifies :math:`T_\mathrm{sys}/\eta`, where :math:`T_\mathrm{sys}` is the antenna system temperature and :math:`\eta` is the antenna efficiency.
