import numpy as np
from simms.constants import PI, C, FWHM_E
...
class Makems:
    def simsky(self, uvw, lm, source_type, gauss_shape, nchan, f0, df, spectrum, dtype):
        ...
        scaled_freq = frequencies * frequencies.dtype.type(gauss_scale)  # multiply the frequency by gauss scale
        ...
        for s in range(nsrc):
            ...
            if source_type[s] == "POINT":
                ...
                raise ValueError("source_type must be POINT or GAUSSIAN")
    ...
    def writems(self):
        ...
        for row in range(nrow):
            vischan[row, :, 0] = self.addsky(uvw, lm, source_type, gauss_shape, nchan, f0, df, spectrum, dtype)
