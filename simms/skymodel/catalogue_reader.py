        ...
        for line in stdr.readlines():
            ...
            if len(rowdata) != len(header):
                raise CatalogueError(
                    "The number of elements in one or more rows does not equal the\
    ...
    for col in ["name", "ra", "dec", "stokes_i"]:
        if not mapcols[col][1]:  # if the list storing column's data is empty
            if col == "name":  # user might not understand what simply "name" means
                raise CatalogueError(
                    "Failed to identify required column corresponding to source name/ID in the catalogue."
...
def dict2sources(data):
    ...
    for i in range(num_sources):
        source = CatSource(
            ...
            transient_start=(data["transient_start"][1][i] if i < len(data["transient_start"][1]) else None),
            transient_absorb=(data["transient_absorb"][1][i] if i < len(data["transient_absorb"][1]) else None),
            transient_ingress=(data["transient_ingress"][1][i] if i < len(data["transient_ingress"][1]) else None),
            transient_period=(data["transient_period"][1][i] if i < len(data["transient_period"][1]) else None),
        )
